...
## `link_or_copy` removes a good output before it replaces it

`common/_cutout_cache.py`'s `link_or_copy` does `if dst.exists(): dst.unlink()`
and then `os.link`, so a present and correct cutout is destroyed before anything
takes its place. Nothing is lost when it works — the cache entry is a separate
directory entry to the same inode and stays authoritative — but on 2026-09-13 an
AB cutout vanished in exactly that window and the rule failed with
`FileNotFoundError` on its own output.

Two changes belong together:

1. Link to a temporary name in the same directory and `os.replace` it over the
   destination, so the output is never absent.
2. Narrow the `except OSError` around `os.link`. It currently also catches
   `FileExistsError`, which would log "could not hardlink — copying instead" and
   then copy a multi-gigabyte file for no reason. It should catch the
   cross-device and not-supported cases its warning actually describes.

Not urgent: with the mtime inversion fixed (the materialised file is now touched,
so it is newer than its inputs) the rule stops re-running on every DAG build, and
the exposure window closes with it. Narrowing the except is also the wrong change
to make while a run is in flight — it converts a currently-caught error into a
crash.
