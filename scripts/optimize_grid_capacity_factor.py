...
grid_cost = 29.7 # EUR/MWh Source: Eurostat. For Germany only (temp), will be a lookup later.
...
fixed_costs = (annual_operations_and_maintenance + annutitized_capital_cost) / electrolyzer_efficiency * 1000 # EUR/MWh H2 LHV
...
def reorder_columns(dfCountry):
    macro_data = ['price', 'demand_forecast', 'wind_onshore_forecast', 'wind_offshore_forecast', 'wind_forecast', 'solar_forecast', 'vre_forecast', 'residual', 'demand']
    imports = [c for c in dfCountry.columns if c.startswith('from') and c != 'from_sum'] + ['from_sum']
    exports = [c for c in dfCountry.columns if c.startswith('to') and c !='to_sum'] + ['to_sum']
    consumption = [c for c in dfCountry.columns if c.endswith('_cons')]
    time_data = ['dt', 'hour', 'doy', 'doy_season']

    new_columns = time_data + macro_data + sorted(imports) + sorted(exports) + sorted(consumption)
...
def add_levelized_capex_cols(df_in, factors):
    levelized_capex_df = df_in.capacity_factor.apply(lambda x: factors / (len(df_in)* x))
    return df_in.join(levelized_capex_df)

def add_total_cost_cols(df_in, cols):
    total_cost_df = (df_in.loc[:, cols].apply(lambda x: x + df_in.average_cost)) \
                    .rename(columns=lambda c: f'{c}_total')
    return df_in.join(total_cost_df)

def calculate_LCOH(dfCountry, year, fixed_costs):
    dfLCOH = (dfCountry.copy()
        .loc[year, ['price']]
        .resample('1h').first()
        .dropna()
        .sort_values('price')
        .assign(variable_cost = lambda df: (df.price+grid_cost)/electrolyzer_efficiency) # grid cost also needs to account for inefficiency
        .assign(average_cost = lambda df: df.variable_cost.cumsum()/np.arange(1, len(df)+1)) # sorted price -> summing as we use more and more hours of the year + dividing by number of hours we use -> average for that capacity factor/hours of the year used
...
def get_perfect_foresight_results(dfLCOH):
    ...
    for r in results.iterrows(): 
        ...
        data = r[1]
        optimum = data
        optimum = optimum[['price', 'variable_cost', 'average_cost', 'capacity_factor', index.split('_')[0], index]]
        optima[f'capital_cost_{index.split('_')[0]}'] = optimum.rename({index.split('_')[0]: 'fixed_cost', index: 'total_cost'})
...
def get_heuristic_results(previous_year_dfLCOH, dfLCOH, cost_decrease_expectation_factor = 0.9):
    ...
    for r in previous_optima.iterrows():
        ...
        optimum = optimum[['price', 'variable_cost', 'average_cost', 'capacity_factor', index.split('_')[0], index]]
        heuristic_optima[f'capital_cost_{index.split('_')[0]}'] = optimum.rename({index.split('_')[0]: 'fixed_cost', index: 'total_cost'})
...
def get_baseload_results(dfLCOH, baseload_cf=1):
    ...
    common_values = pd.concat([dfLCOH.loc[m].loc[['price', 'variable_cost', 'average_cost', 'capacity_factor']]]*3, axis=1)
    ...
    total_cost_row = dfLCOH.loc[m].loc[[f"{c}_total" for c in fixed_costs.index]].to_frame().T
