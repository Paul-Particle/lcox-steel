...
OUTDIR = Path("results/plots/bestsite_locations_from_metadata")
...
def read_location_metadata(path: Path) -> dict:
    """
    Read selected-cell coordinates from the first row of a CF CSV.
    """
    ...
    for tech in TECHS:
        meta[tech] = {
            "x": float(df[f"{tech}_x"].iloc[0]),
            "y": float(df[f"{tech}_y"].iloc[0]),
            "x_idx": int(df[f"{tech}_x_idx"].iloc[0]),
            "y_idx": int(df[f"{tech}_y_idx"].iloc[0]),
        }
...
def find_res_mix_metadata_files(iso2: str) -> list[Path]:
    """
    Find anchor-based RES-mix/location files created by script 07.

    Includes:
    - generic land-anchor files
    - scenario-specific offshore-anchor mix files
    """
    base = Path("data/res_cf")
    cc = iso2.lower()

    files = []

    files.extend(base.glob(f"{cc}_cf_{YEAR}_bestsite_p95_anchor-wind_onshore.csv"))
    files.extend(base.glob(f"{cc}_cf_{YEAR}_bestsite_p95_anchor-solar.csv"))
    files.extend(base.glob(f"{cc}_cf_{YEAR}_bestsite_p95_anchor-wind_offshore_mix-*.csv"))
...
def read_anchor_metadata(path: Path) -> tuple[str, str]:
    ...
    anchor_tech = str(df["anchor_tech"].iloc[0])
    mix_label = df["mix_label"].iloc[0]

    if pd.isna(mix_label):
        mix_label = ""

    return anchor_tech, str(mix_label)
...
def best_and_p95_threshold(cell_mean: xr.DataArray, geom):
    ...
    return best_mask, p95_threshold
...
def plot_bestsite_map(iso2: str, tech: str) -> None:
    ...
    # Best cell still computed from annual mean grid for visual comparison.
    # P95 location itself comes from metadata below.
    best_mask, p95_threshold = best_and_p95_threshold(cell_mean_select, geom)

    meta_path = Path(f"data/res_cf/{iso2.lower()}_cf_{YEAR}_bestsite_p95.csv")
    meta = read_location_metadata(meta_path)

    p95_mask = np.zeros(cell_mean.shape, dtype=bool)
    p95_y_idx = meta[tech]["y_idx"]
    p95_x_idx = meta[tech]["x_idx"]
    p95_mask[p95_y_idx, p95_x_idx] = True    
    xs = cell_mean.coords["x"].values
    ys = cell_mean.coords["y"].values
    ...
    out = OUTDIR / f"{iso2.lower()}_{tech}_bestsite_from_metadata.png"
    fig.tight_layout()
...
def plot_res_mix_locations(iso2: str, path: Path) -> None:
    ...
    # land boundary
    land_boundary = gpd.GeoSeries([load_land_geometry(iso2)], crs=4326)
    land_boundary.boundary.plot(ax=ax, color="black", linewidth=1.5)

    # offshore boundary (if exists)
    try:
        offshore_boundary = gpd.GeoSeries([load_offshore_geometry(iso2)], crs=4326)
        offshore_boundary.boundary.plot(ax=ax, color="black", linewidth=1, linestyle="--")
    except:
        pass
