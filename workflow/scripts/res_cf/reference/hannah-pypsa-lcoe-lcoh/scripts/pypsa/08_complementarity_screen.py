...
Both outputs feed directly into script 09 (PyPSA optimisation).
...
def main() -> None:
    ...
    for country in cfg["countries"]:
        ...
        for tech in TECHS:
            ...
            valid_cfs   = mean_vals[valid]
            threshold_cf = np.nanpercentile(valid_cfs, cfg["quality_floor"] * 100)
            qualified   = valid #& (mean_vals >= threshold_cf)

            ys, xs = np.where(qualified)
            n      = len(ys)
            print(f"    {tech}: {n} candidate cells (all valid cells)")
            ...
            candidates[tech]   = (ys, xs, mean_vals[qualified])
        ...
        print(f"  Sample onshore coords: lon={lons_on[0]:.2f}, lat={lats_on[0]:.2f}")
        print(f"  Sample offshore coords: lon={lons_off[0]:.2f}, lat={lats_off[0]:.2f}")
        print(f"  Sample solar coords: lon={lons_sol[0]:.2f}, lat={lats_sol[0]:.2f}")
        test_dist = max_pairwise_distance_km_coords(
            lons_on[0], lats_on[0],
            lons_off[0], lats_off[0],
            lons_sol[0], lats_sol[0],
        )
        print(f"  Sample triplet max distance: {test_dist:.1f} km")

        # DIAGNOSTIC - count valid onshore-offshore pairs within radius
        count_on_off = 0
        for i in range(len(lons_on)):
            for j in range(len(lons_off)):
                d = haversine_distance_km(
                    lons_on[i], lats_on[i],
                    np.array([lons_off[j]]), np.array([lats_off[j]])
                )[0]
                if d <= cfg["max_radius_km"]:
                    count_on_off += 1
        print(f"  Valid onshore-offshore pairs within {cfg['max_radius_km']}km: {count_on_off}")
        ...
        # PLAUSIBILITY PLOT - remove after checking
        import matplotlib.pyplot as plt

        best = top_records[0]
        ts_on  = ts_matrices["wind_onshore"][:,  candidates["wind_onshore"][0].tolist().index(best["onshore_y_idx"])]
        fig, ax = plt.subplots(figsize=(14, 4))
        # plot first 2 weeks
        hours = range(24*14)
        ax.plot(list(hours), ts_matrices["wind_onshore"][:24*14, 
            np.where((ys_on == best["onshore_y_idx"]) & (xs_on == best["onshore_x_idx"]))[0][0]], 
            label="onshore wind", alpha=0.8)
        ax.plot(list(hours), ts_matrices["wind_offshore"][:24*14,
            np.where((ys_off == best["offshore_y_idx"]) & (xs_off == best["offshore_x_idx"]))[0][0]],
            label="offshore wind", alpha=0.8)
        ax.plot(list(hours), ts_matrices["solar"][:24*14,
            np.where((ys_sol == best["solar_y_idx"]) & (xs_sol == best["solar_x_idx"]))[0][0]],
            label="solar", alpha=0.8)
        ax.set_title(f"AUS best triplet - 2 weeks sample (lon={best['onshore_x']}, lat={best['onshore_y']})")
        ax.legend()
        plt.tight_layout()
        plt.savefig("results/plots/aus_best_triplet_check.png", dpi=150)
        print("  Plausibility plot saved → results/plots/aus_best_triplet_check.png")
