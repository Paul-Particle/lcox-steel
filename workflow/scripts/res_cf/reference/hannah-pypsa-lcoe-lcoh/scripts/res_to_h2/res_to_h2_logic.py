...
def annual_cost_res_mix(
    total_res_capacity_mw: float,
    scenario: dict,
    wacc: float,
) -> float:
    """
    Annualised RES cost for a fixed RES mix.
    For now, uses the same dummy RES config for each tech separately,
    but still sums per-tech costs to preserve correct structure for later.
    """
    ...
    for mix_tech, weight in res_mix.items():
        tech_capacity_mw = float(total_res_capacity_mw) * float(weight)
        total_cost += annual_cost_res(tech_capacity_mw, res_cfg, wacc)
