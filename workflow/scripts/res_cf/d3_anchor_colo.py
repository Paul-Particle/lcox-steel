...
Outputs
-------
resources/timeseries/{area}_{tech}_anchor-colo-n{N}_{start_date}_{end_date}.parquet
...
_AREA = "DE_LU"
_REGION = "DE"   # region tag stored in the geo parquet
...
_OUT = TIMESERIES / f"{_AREA}_{_ANCHOR_TECH}_anchor-colo-n{_N_CANDIDATES}_{_START_DATE}_{_END_DATE}.parquet"
...
def build_anchor_scenario(cutout_path: Path, _AREA: str, anchor_tech: str,
    ...
    """
    anchor_geom = geometry_for_tech(_AREA, anchor_tech)
    ...
    for tech in other_techs:
        ...
        tech_geom = geometry_for_tech(_AREA, tech)
        ...
        if not found:
            log.warning(
                f"{_AREA}: no valid {tech} cell within "
...
def main() -> None:
    ...
    anchor_series, anchor_xy, candidates_meta, candidates_series = build_anchor_scenario(
        _CUTOUT_PATH, _REGION, anchor_tech,
