...
def plot(sites: pd.DataFrame, out: Path, scenario: str, route: str) -> None:
    ...
    apply_header(
        fig,
        title=f"{scenario} / {route} — built capacity by site",
...
def main() -> None:
    ...
    if int((n.buses.carrier == "AC").sum()) <= 1:
        raise ValueError(
            f"{scenario}/{route} is a single-site network — plot_site_capacity_bars "
            "is only meaningful for multi-site scenarios (those with a sites overlay)."
        )
    plot(build_site_table(n), Path(snakemake.output.png), scenario, route)
