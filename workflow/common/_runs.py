...
# The market of an area whose series is made up by the workflow rather than
# downloaded. It exists so the demo can price an export furnace without a key,
# and is never a place a real scenario covers.
...
def top_level_areas(areas: dict) -> list[str]:
    """The areas `all-areas` starts from — every real area that is not someone's zone."""
    nested = {zone for cfg in areas.values() for zone in cfg.get("zones", [])}
    synthetic = {area for area, cfg in areas.items()
                 if cfg.get("market") == SYNTHETIC_MARKET}
    return sorted(set(areas) - nested - synthetic)
